...
std::string SetPointProcessing::compute()
{
	read();
	removeOutlier(removeOutlierPercentage, removeOutlierNeighbor);
	wlopSimplify(wlopSimplifyPercentage, wlopSimplifyNeighbor);
	smooth(smoothNeighbor);
    	write();

	readPca();
	pcaEstimateNormals(pcaEstimateNormalsNeighbor);
	writePca();
...
void SetPointProcessing::removeOutlier(const double removedPercentage, unsigned int neighborNumber)
{
...
void SetPointProcessing::wlopSimplify(const double retainPercentage, const double neighborRadius)
{
